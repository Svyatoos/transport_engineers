...
def list_available_cameras(max_cameras=10):
    ...
    for i in range(max_cameras):
        ...
        if cap.isOpened():
            ...
            cap.release()  # Освобождаем ресурсы после проверки
    return available_cams

def list_usb_ports():
    """Функция для получения списка подключенных USB портов."""
    ports = list_ports.comports()
    usb_ports = {}
    for port in ports:
        usb_ports[port.device] = port.description
    return usb_ports
...
print("Подключенные USB порты:")
for port, description in usb_ports.items():
...
while True:
    try:
        ...
        if camera_index in cameras:
            ...
            print("Выбранный индекс камеры недоступен.")
    except ValueError:
...
class MyVideoCapture:
    ...
    def get_frame(self):
        if self.vid.isOpened():
            ...
            if ret:
                # Возвращаем флаг успеха и текущий кадр, преобразованный в RGB
...
class App:
    def __init__(self, window, window_title, video_source=0):
        ...
        # Устанавливаем задержку обновления
        self.delay = 15
        self.update()
...
# Кнопки управления
btn_start = Button(frame1, text='Старт', bg='gray', command=lambda: App(root, "Программа для детектирования лица"))
btn_start.place(relx=0.865, rely=0.034, relheight=0.05, relwidth=0.13)

btn_stop = Button(frame1, text='Стоп', bg='gray', command=bbtg2)
btn_stop.place(relx=0.865, rely=0.089, relheight=0.05, relwidth=0.13)

btn_add_robot = Button(frame1, text='Добавить фоторобот', bg='gray', command=bbtg3)
btn_add_robot.place(relx=0.865, rely=0.144, relheight=0.05, relwidth=0.13)

btn_analyze = Button(frame1, text='Анализ изображения', bg='gray', command=bbtg4)
btn_analyze.place(relx=0.865, rely=0.199, relheight=0.05, relwidth=0.13)
